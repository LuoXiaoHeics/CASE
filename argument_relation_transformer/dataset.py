...
discourse_makers = ['actually','in fact', 'but', 'specifically','in particular','particularly','due to','for this reason','consequently', \
        'hence','thus','nonetheless','nevertheless','though','although','therefore','however','because']
...
class ArgumentRelationDataset(Dataset):
    ...
    def _load_data(self):
        ...
        for ln in open(path):
            ...
            if self.set_type == 'train':
                self._include_bidir_context_argument(cur_obj)
    ...
    def _include_bidir_context_argument(self, doc_obj):
        """Create actual training sample in sequences, by extracting context from left and right.
        ...
        doc_id = str(doc_obj["doc_id"])+'arg'
        cur_sents = doc_obj["text"]

        for i in range(len(cur_sents)):
            sent = cur_sents[i].lower()
            for u in discourse_makers:
                if u in sent:
                    if np.random.rand()>0.4:
                        sent = sent.replace(u,' <mask> ')
            cur_sents[i] = sent
        for i in range(len(cur_sents)):
            if np.random.rand()>0.85:
                cur_sents[i] = '<mask> <mask> <mask> <mask> <mask> <mask> '

        # if len(doc_obj["text"])!= len(cur_sents):
        #     print('????????????????????????')
    ...
    def collater(self, samples):
        ...
        for ix, s in enumerate(samples):
            ...
            for u in disc_token_pos:
                sentence_cls_mask[ix][u] = 1     
        ...
        batch["sentence_cls_mask"] = torch.LongTensor(sentence_cls_mask)
