...
def main():
    ...
    parser.add_argument(
        "--gpu-id", type=int, help="Assign GPU", default=0,
    )
    ...
    parser.add_argument("--scheduler-type", type=str, choices=["linear", "constant"],default='constant')
    ...
    parser.add_argument("--cl", type=float, default=0.001)

    args = parser.parse_args()
    set_seeds(args.seed)

    if args.dataset in ["echr", "cdcp",'abst_rct']:
    ...
    trainer_args['devices'] = [args.gpu_id]
    ...
    run_evaluation(
        output_path=os.path.join(args.output_dir, f"{args.exp_name}.jsonl"),
